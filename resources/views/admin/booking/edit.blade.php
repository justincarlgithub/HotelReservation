...
@section('title') {{'$room->name'}} @endsection
  ...
  <div class="row auto">
    ...
    <div class="col-md-8">
      ...
      @if ($room->check_in >= $newdate)
      <h1><span style="font-size: 25px;" class="pb-3"><a class="fa-solid fa-chevron-left" href="{{url('admin/booking-checkin')}}" style="color: #060e4d;"></a> </span> {{$room->name}}</h1>
      @else
      <h1><span style="font-size: 25px;" class="pb-3"><a class="fa-solid fa-chevron-left" href="{{url('admin/booking')}}" style="color: #060e4d;"></a> </span> {{$room->name}}</h1>
      ...
      <div class="position-sticky border border-dark rounded" style="top: 2rem;">
        ...
        <div class="px-4">
            ...
            <div class="form-row">
              <div class="control-group col-12">
                ...
                @if ($room->check_in >= $newdate)
                  ...
                  <input type="date" name="check_in" id="check_in" class="form-control" min="<?php echo date("Y-m-d"); ?>" value="{{ $roomreservation->check_in }}" readonly >
                  <span id="error_checkin"></span> 
                @else
                <label>Check-In</label>
                <input type="date" name="check_in" id="check_in" class="form-control" min="<?php echo date("Y-m-d"); ?>" oninput="myFunction3()" value="{{ $roomreservation->check_in }}">
                <span id="error_checkin"></span>
              </div>
              ...
              <div class="control-group col-12">
                  ...
                  <input type="date" name="check_out" id="check_out" class="form-control" value="{{ $roomreservation->check_out}}" oninput="myFunction2()" />
          ...
          <div class="control-group">
            ...
            <input type="hidden" value={{ $room->id}} name="room" id="room">
         <center>
          <input style="background-color:#060e4d; color:#fff;"type="button" class="btn" data-bs-toggle="modal" href="#exampleModalToggle"  id="su" value="Reserve" >
...
function myFunction3() {
  ...
  var x = document.getElementById("check_out").value;
    ...
    var _token = $('input[name="_token"]').val();
     $.ajax({
      url: "{{url ('editdatecheck') }}",
      method:"POST",
      data:{check_in:check_in,room_id:room_id, room:room, _token:_token},
      success:function(result)
      {
        if (z >= x)
          {
            $('#error_checkout').html('<label class="text-danger"><b>Date invalid &nbsp; &#9888;</b></label>');
  ...
  function myFunction2() {
    ...
    var _token = $('input[name="_token"]').val();
     $.ajax({
      url: "{{url ('editoutdate') }}",
      method:"POST",
      data:{check_in:check_in,check_out:check_out,room:room,room_id:room_id, _token:_token},
      success:function(result)
      {
