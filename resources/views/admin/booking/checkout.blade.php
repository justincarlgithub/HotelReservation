                    ...
                    <tr>
                      ...
                      <td>
                        @if($value->status==0)
                          ...
                          <center><span style="color: red"><b>REDTAGGED</b></span></center>
                           @else 
                          <center><span style="color:red">-----------------------</span></center>
                          @endif 
                        @endif
                        @if($value->status==2)
                        <center><span style="color:green"><b>Checkout</b></span></center>
                        @endif
                        @if($value->status==1)
                        <center><span style="color:red"> <a href="{{ url('confirm-password/'.$value->id) }}"><b>Checkin</b></a></center>
                        @endif
                      </td>
                      ...
                      <td> 
                        @if($value->status ==1) 
                        <center> <a href="{{url('admin/updatestatus2/'.$value->id)}}" onclick="confirmation(event)" class="btn btn-info">UNPAID</a></center>
                       
                        @elseif ($value->status ==2)
                        <center> <a class="btn btn-success"> PAID </a></center>
                       
                        @else
                        <center><a href="{{ url('confirm-password/'.$value->id) }}" class="btn btn-sm btn-info " style="pointer-events: none;">UNPAID</a></center>
                        
                        @endif
                       </td> 
                      @endif
                      <td>
                          <center><a href="booking/delete/{{$value->id}}" onclick="confirmation1(event)" class="btn btn-danger"><i class="fa-solid fa-trash"></i></a></center> 
